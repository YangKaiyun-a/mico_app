...
void VirtualDisplay::parentItemChange(GraphicsItemChange change, const QVariant &value)
{
    switch (change)
    {
...
void VirtualDisplay::SetPoseInParent(const RobotPose &pose)
{
    ...
    QPointF scene_pose = parent_ptr_->mapToScene(QPointF(pose_in_parent_.x, pose_in_parent_.y));
    setPos(scene_pose);
}

bool VirtualDisplay::SetDisplayConfig(const std::string &config_name, const std::any &config_data)
{
    return true;
}
...
std::string VirtualDisplay::GetDisplayType()
{
...
void VirtualDisplay::wheelEvent(QGraphicsSceneWheelEvent *event)
{
    ...
    if (event->delta() > 0)
    {
        MovedBy(-event->pos().x() * beforeScaleValue * 0.1, -event->pos().y() * beforeScaleValue * 0.1);
    }
    else
    {
        MovedBy(event->pos().x() * beforeScaleValue * 0.1, event->pos().y() * beforeScaleValue * 0.1);
