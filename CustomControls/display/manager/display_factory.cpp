...
void FactoryDisplay::RemoveDisplay(VirtualDisplay *display)
{
    ...
    auto parent_ptr_ = FactoryDisplay::Instance()->GetDisplay(display->GetParentName());
...
void FactoryDisplay::AddDisplay(VirtualDisplay *display, const std::string &parent_name)
{
    ...
    {
        std::cout << "display name:" << display->GetDisplayName() << " already exist" << std::endl;
        return;
    }
...
// 设置响应鼠标事件的图层
bool FactoryDisplay::SetMoveEnable(const std::string &display_name, bool enable)
{
    if (total_display_map_.count(display_name) != 0)
    {
