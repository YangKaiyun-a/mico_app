...
PointShape::PointShape(const ePointType &type, const std::string &display_type, const std::string &display_name, const int &z_value, std::string parent_name)
    : VirtualDisplay(display_type, z_value, parent_name, display_name), type_(type)
{
    ...
    switch (type_)
    {
        case kRobot:
        {
            ...
            deg_offset_ = 45;
            SetBoundingRect(QRectF(0 - robot_svg_renderer_.defaultSize().width() / 2, 0 - robot_svg_renderer_.defaultSize().height() / 2,
                                   robot_svg_renderer_.defaultSize().width(), robot_svg_renderer_.defaultSize().height()));
            break;
        }
        case kParticle:
        {
...
QVariant PointShape::itemChange(GraphicsItemChange change, const QVariant &value)
{
    switch (change)
    {
        case ItemPositionHasChanged:
            curr_scene_pose_ = RobotPose(scenePos().x(), scenePos().y(), normalize(robot_pose_.theta + rotate_value_));
...
bool PointShape::SetDisplayConfig(const std::string &config_name, const std::any &config_data)
{
...
void PointShape::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    {
        curr_scene_pose_ = RobotPose(scenePos().x(), scenePos().y(), normalize(robot_pose_.theta + rotate_value_));
        emit signalPoseUpdate( RobotPose(scenePos().x(), scenePos().y(), normalize(robot_pose_.theta + rotate_value_)));
    }
}
