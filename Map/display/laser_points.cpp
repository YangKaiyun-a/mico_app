...
LaserPoints::LaserPoints(const std::string &display_type, const int &z_value, std::string parent_name)
    : VirtualDisplay(display_type, z_value, parent_name)
{

}

void LaserPoints::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
...
void LaserPoints::computeBoundRect(const std::map<int, std::vector<Point>> &laser_scan)
{
...
bool LaserPoints::SetDisplayConfig(const std::string &config_name, const std::any &config_data)
{
    return true;
}

void LaserPoints::drawLaser(QPainter *painter, int id, std::vector<Point> data)
{
    QColor color;
