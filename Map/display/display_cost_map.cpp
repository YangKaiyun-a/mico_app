...
DisplayCostMap::DisplayCostMap(const std::string &display_type, const int &z_value, std::string parent_name)
...
{
    Eigen::Matrix<Eigen::Vector4i, Eigen::Dynamic, Eigen::Dynamic> cost_map = cost_map_data_.GetCostMapData();
    map_image_ = QImage(cost_map_data_.Cols(), cost_map_data_.Rows(), QImage::Format_ARGB32);
...
bool DisplayCostMap::SetDisplayConfig(const std::string &config_name, const std::any &config_data)
{
    update();
    return true;
}

void DisplayCostMap::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    //以图片中心做原点进行绘制(方便旋转)
