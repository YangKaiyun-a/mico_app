...
NavGoalWidget::NavGoalWidget(QWidget *parent) : QWidget(parent)
{
    ...
    connect(button_cancel, &QPushButton::clicked, [this]() {
        emit SignalHandleOver(HandleResult::kCancel, RobotPose(spinBox_x_->value(), spinBox_y_->value(), deg2rad(spinBox_theta_->value())));
    });
    connect(button_remove, &QPushButton::clicked, [this]() {
        emit SignalHandleOver(HandleResult::kRemove, RobotPose(spinBox_x_->value(), spinBox_y_->value(), deg2rad(spinBox_theta_->value())));
    });
}
