...
bool rclcomm::Start()
{
    ...
    node = rclcpp::Node::make_shared("mico_ros2_node");
    ...
    // for (auto one_image_display : ConfigManager->GetRootConfig().images)
    // {
    //     LOG_INFO("image location:" << one_image_display.location << "topic:" << one_image_display.topic);

    //     image_subscriber_list_.emplace_back(
    //         node->create_subscription<sensor_msgs::msg::Image>(one_image_display.topic, 1, [this, one_image_display](const sensor_msgs::msg::Image::SharedPtr msg) {
...
void rclcomm::map_callback(const nav_msgs::msg::OccupancyGrid::SharedPtr msg)
{
    ...
    occ_map_ = basic::OccupancyMap(height, width, Eigen::Vector3d(origin_x, origin_y, 0), resolution);
...
void rclcomm::localCostMapCallback(const nav_msgs::msg::OccupancyGrid::SharedPtr msg)
{
    ...
    basic::OccupancyMap cost_map(height, width,Eigen::Vector3d(origin_x, origin_y, 0),msg->info.resolution);
...
basic::RobotPose rclcomm::getTransform(std::string from, std::string to)
{
    basic::RobotPose ret;
    try {
        geometry_msgs::msg::TransformStamped transform = tf_buffer_->lookupTransform(to, from, tf2::TimePointZero);
