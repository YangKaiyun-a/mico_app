...
struct MapConfig {
    ...
    bool Load(const std::string &filename)
    {
        ...
        if (fin.fail())
        {
            std::cout << "Map_server could not open " << filename.c_str();
            return false;
        }
...
public:
    OccupancyMap() {}
    OccupancyMap(int rows_, int cols_, Eigen::Vector3d origin, double res) : map_config({origin[0], origin[1], origin[2]}, res)
        , rows(rows_), cols(cols_), map_data(rows_, cols)
    {

    }

    OccupancyMap(int rows_, int cols_, Eigen::Vector3d origin, double res, Eigen::MatrixXi data) : map_config({origin[0], origin[1], origin[2]}, res)
        , rows(rows_), cols(cols_), map_data(data)
    {
